          ...
          <div class="col-xl-7 pl15-xl pl0 mb35">
            <div class="blog_grid_post style2 event_lists">
              ...
              if($this->content->title){
                $this->content->text .='<h3>'.format_text($this->content->title, FORMAT_HTML, array('filter' => true)).'</h3>';
              }
              if($this->content->body){
                $this->content->text .='<p>'.format_text($this->content->body, FORMAT_HTML, array('filter' => true)).'</p>';
              }
              $this->content->text .='
                <ul class="mb0">';
                  ...
                  if($this->content->time){
                    $this->content->text .='<li class="ccn-block-featured-event-detail"><span class="flaticon-clock"></span>'.format_text($this->content->time, FORMAT_HTML, array('filter' => true)).'</li>';
                  }
                  if($this->content->location){
                    $this->content->text .='<li class="ccn-block-featured-event-detail"><span class="flaticon-placeholder"></span>'.format_text($this->content->location, FORMAT_HTML, array('filter' => true)).'</li>';
                  }
                  $this->content->text .='
                </ul>';
                if(!empty($this->content->button_text)){
                  $this->content->text .='<a href="'.$this->content->button_link.'" class="btn dbxshad btn-md btn-thm2 rounded mt30">'.$this->content->button_text.'</a>';
                }
                $this->content->text .='
              </div>
            </div>
