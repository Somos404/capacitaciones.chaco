...
class block_cocoon_course_rating extends block_base {
    ...
    public function get_content() {
        ...
        if(!empty($this->config->title)){$this->content->title = '<h4 class="aii_title">'. format_text($this->config->title, FORMAT_HTML, array('filter' => true)) .'</h4>';} else {$this->content->title = '<h4 class="aii_title">'.get_string('pluginname', 'block_cocoon_course_rating').'</h4>';}
      ...
      $return =   '<form id="ccn-star-rate" method="post" action="'.$CFG->wwwroot.'/theme/edumy/ccn/form_handler/ccn_rate_course.php">
                    ...
                    <div class="ccn-star-rate-inner">';
                      for ($i = 5; $i >= 1; $i--) {
                        ...
                        $return .='    <input type="radio" id="stars-'.$i.'" name="rating" value="'.$i.'" /><label for="stars-'.$i.'"></label>';
                      }
      $return .= '  </div>
